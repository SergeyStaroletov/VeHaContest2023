...
mtype = {accelon, acceloff, dvigon, dvigoff, reboot, nul, v , vdvig, other, ans};
chan cch = [1] of {mtype};
chan dch = [1] of {mtype};
int answer =0;
...
do
  ::
	if
		...
		::(state ==2) && (nfull(poolout)) -> {poolout!ans; printf("BIUS: send ans\n"); state=1}
...
do
	...
	::(nempty(bkupoolin)) -> {bkupoolin ? bkudt;
		if
			...
			:: (bkudt==ans) ->  {printf("BKU: get answer\n"); answer=1}
	...
	::(state==0) && (len(bkupoolout)<16)-> { bkupoolout!accelon; printf("BKU: start accel\n"); state=1};
	::(state==1) && (nempty(bkupoolout)) && (nfull(cch))-> {bkupoolout ? bkucommand; 
		if
			::(bkucommand==accelon) -> {printf("BKU: send accelon\n"); sendcom=2}
		fi;
		cch!bkucommand; state=2};
		
	::(state==2) && (answer==1) &&(len(bkupoolout)<16) -> { bkupoolout!dvigon; printf("BKU: start dvig\n"); state=3};
	::(state==3) && (nempty(bkupoolout)) && (nfull(cch))-> {bkupoolout ? bkucommand; 
		if
			::(bkucommand==dvigon) -> {printf("BKU: send dvigon\n"); sendcom=3}
		fi;
		cch!bkucommand; trans_run =1};
